...
export default function SharingSection({ results }) {
  ...
  const message = buildClientMessage(results);
  ...
  const handleTelegram = async () => {
    ...
    markEstimateSent({ uuid: results.uuid, channel: "Telegram" });
    window.open(tgUrl, "_blank", "noopener,noreferrer");
  };

  const handleWhatsApp = () => {
    markEstimateSent({ uuid: results.uuid, channel: "WhatsApp" });
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4 sm:p-5 space-y-3">
      <p className="text-sm text-gray-700 text-center">
        Чтобы получить подробный расчет стоимости и обсудить детали, отправьте
        смету удобным для Вас способом:
      </p>
      <div className="flex gap-3">
        <button
          onClick={handleTelegram}
          className="flex-1 flex items-center justify-center gap-2 bg-sky-500 hover:bg-sky-600 text-white font-semibold py-3 rounded-xl transition"
        >
          <svg
            viewBox="0 0 24 24"
